...
lazy_static! {
    static ref GET_COUNTER: RwLock<u32> = RwLock::new(0);
    static ref POST_COUNTER: RwLock<u32> = RwLock::new(0);
    static ref DELETE_COUNTER: RwLock<u32> = RwLock::new(0);
    static ref TOTAL_COUNTER: RwLock<u32> = RwLock::new(0);
}   

fn handle_request(req: Request) -> String{

    let mut total_counter = TOTAL_COUNTER.write().unwrap();
...
pub fn main() {
    ...
    let get_counter = GET_COUNTER.read().unwrap();

    let post_counter = POST_COUNTER.read().unwrap();

    let delete_counter = DELETE_COUNTER.read().unwrap();
    
    let total_counter = TOTAL_COUNTER.read().unwrap();
